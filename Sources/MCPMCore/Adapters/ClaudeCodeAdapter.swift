...
public struct ClaudeCodeAdapter: ClientAdapter {
    ...
    public init(configPath: URL = HomeDirectory.url.appendingPathComponent(".claude.json")) {
        self.configPath = configPath
    }
    public func isInstalled() -> Bool {
        FileManager.default.fileExists(atPath: configPath.path)
            || FileManager.default.fileExists(atPath: HomeDirectory.url.appendingPathComponent(".claude").path)
    }
